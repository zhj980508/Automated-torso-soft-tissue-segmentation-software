...
for i in range(20,int(img_shape[1]-1)-20):        
...
data_HU_SoftTissue_Color = cv.cvtColor(data_CTBedRemoval.astype(np.uint8), cv.COLOR_GRAY2RGB)
...
#cv.imwrite('606_OriginalImage.png', data_HU_SoftTissue_Color)
...
def ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height,distance):
    ...
    # find peak locations -- find concave points
    peak_locx,peak_property = find_peaks(Torso_initial_mask_sum_neg, height=90, distance=200)
    peak_height = peak_property['peak_heights']
    print('peak_locx',peak_locx)
    print('peak_height',peak_height) 
...
Torso_mask = ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height=90,distance=200)
