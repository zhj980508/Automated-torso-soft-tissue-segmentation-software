...
def ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height,distance):
    ...
    peak_locx,peak_property = find_peaks(Torso_initial_mask_sum_neg, height=120, distance=200)
...
files = LoadAllDicomSlicesFromOneFolder(dir_526)
...
for i in range(40,int(img_shape[1]-1)-40):        
...
def ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height,distance):
    ...
    # find peak locations -- find concave points
    peak_locx,peak_property = find_peaks(Torso_initial_mask_sum_neg, height=70, distance=100)
    peak_height = peak_property['peak_heights']
    print('peak_locx',peak_locx)
    print('peak_height',peak_height) 
...
Torso_mask = ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height=70,distance=100)
...
thickness = 4
#cv.circle(data_HU_SoftTissue_Color,(int(cx_SoftTissue[0]),int(cy_SoftTissue[0])),point_size, point_color, thickness)
