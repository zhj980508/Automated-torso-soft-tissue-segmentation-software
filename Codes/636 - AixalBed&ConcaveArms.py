...
# In[]: CT bed removal -- no need for choosing one slice for removal
'''
# Limitation: Cannot remove the bed region where it is connected to the torso
# Is ideal but hard to relise, need to set a lot parameters, not so convenient

# Binarize all slices
SoftTissue_mask = np.zeros([512,512,len(VertebraName_606)])
for i in range(len(VertebraName_606)):
    data_HU_SoftTissue = LoadImageData_HU_SoftTissue(VertebraName_606[i],Vertebra_Filename_606_list,dir_606)   
    ret,SoftTissue_mask[:,:,i] = cv.threshold(data_HU_SoftTissue,0,255,cv.THRESH_BINARY)

# Add them all by numbers
SoftTissue_mask = SoftTissue_mask.astype(np.float)
SoftTissue_mask_sum = np.sum(SoftTissue_mask,axis=2)

# Set another threshold to define the bed region(some range to the max)
SoftTissue_MultipleThres = SoftTissue_mask_sum > 4000
plt.imshow(SoftTissue_MultipleThres)

# CCA to select the bed region
kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE,(2,2))
morph_close = cv.morphologyEx(SoftTissue_MultipleThres.astype(np.uint8), cv.MORPH_CLOSE, kernel) # close operation to remove small spots
labels, num=measure.label(morph_close,connectivity=2,background=0,return_num=True)
...
tmp_list=sorted(NumberofEachArea) # sort the labels
BedRegion_num = tmp_list[-6]
idx = NumberofEachArea.index(BedRegion_num) # label we want
MaxConnectedArea = np.where(labels == idx, 1, 0)
plt.imshow(MaxConnectedArea)

# Remove the bed region
'''
'''
...
files = LoadAllDicomSlicesFromOneFolder(dir_606)
...
TableTop_CTbed = int(min(rho_CTbed)) # This is what we want for bed removal
# Return angle and table top profile on the first transverse slice
AxialImage_Cut = AxialImage
AxialImage_Cut[TableTop_CTbed:AxialImage.shape[0],:] = 0
plt.imshow(AxialImage_Cut)
'''
...
VertebraName_BED = 'T4' #The best compromise
data_HU_BED_SoftTissue = LoadImageData_HU_SoftTissue(VertebraName_BED,Vertebra_Filename_636_list,dir_636)
plt.imshow(data_HU_BED_SoftTissue, cmap='gray') 
plt.title('Original image of C2(636)')
plt.show()
...
plt.title('Original image of T8(636)')
plt.show()
...
def ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height,distance):
    ...
    # find peak locations -- find concave points
    peak_locx,peak_property = find_peaks(Torso_initial_mask_sum_neg, height=500, distance=200,width=0.1)
    peak_height = peak_property['peak_heights']
    print('peak_locx',peak_locx)
    print('peak_height',peak_height) 
...
Torso_mask = ArmRemovalBySearchingConcavePoints(Torso_initial_mask,height=50,distance=100)
...
plt.title('Original image of for eva T8(636)')
plt.show()
...
plt.show()
#cv.imshow('Soft tissue_T8_00636M (Target Region)', data_HU_SoftTissue_Color)  
#cv.waitKey(0)
...
cv.imshow("Soft tissue_C3_00636M (Target Region)",data_HU_SoftTissue_Color)
cv.waitKey(0)
